...
int createUDTSocket(UDTSOCKET& usock, int port = 0, bool rendezvous = false) {
  ...
  // starts with many connections, so the buffer size set to smaller value.
  int snd_buf = 16000;
  int rcv_buf = 16000;
...
int connect(UDTSOCKET& usock, int port) {
  ...
  if (0 != getaddrinfo(g_Localhost, buffer, &hints, &peer))  return -1;
...
#endif
{
  ...
  if (new_sock == UDT::INVALID_SOCK) return NULL;
  ...
  UDTSOCKET readfds[1];
  int num = 1;
  if (UDT::epoll_wait2(eid, readfds, &num, NULL, NULL, -1) > 0) UDT::close(new_sock);
...
#endif
{
  ...
  UDTSOCKET serv;
  if (createUDTSocket(serv, g_Server_Port) < 0) return NULL;
  ...
  // create TCP sockets
  SYSSOCKET tcp_serv;
  if (createTCPSocket(tcp_serv, g_Server_Port) < 0) return NULL;
  ...
  auto i = new_socks.begin(); // vector<UDTSOCKET>::iterator
  for (; i != new_socks.end(); ++i) UDT::close(*i);

  auto i = tcp_socks.begin(); // vector<SYSSOCKET>::iterator 
...
#endif
{
  ...
  // The first shared port has been created, so start from g_IndUDTNum + 1.
  ...
  auto i = cli_socks.begin(); // vector<UDTSOCKET>::iterator
  ...
  tcp_socks.resize(g_TCPNum);
  for (int i = 0; i < g_TCPNum; ++i) {
    if (createTCPSocket(tcp_socks[i], 0) < 0) return NULL;
...
#endif
{
  ...
  auto i = srv_socks.begin(); // vector<UDTSOCKET>::iterator
...
#endif
{
  ...
  for (int i = 0; i < g_UDTNum3; ++i) {
    if (createUDTSocket(cli_socks[i], port++, true) < 0) cout << "error\n";
  }

  int peer_port = 61000;

  auto i = cli_socks.begin();
...
#endif
{
  ...
  auto i = cli_threads.begin(); // vector<pthread_t>::iterator 
  ...
  auto i = cli_threads.begin(); // vector<HANDLE>::iterator
  for (; i != cli_threads.end(); ++i) {
